...
interface City2DCanvasProps {
  ...
  onRoadDragEnd?: (x: number, y: number) => void;
...
export function City2DCanvas({
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const [x, y] = coords;
    setCursorPos([x, y]);
    // Keep the map scroll position stable when the hidden roving focus target
    // receives focus. Without preventScroll, a click on a scrolled tile can
    // jump back to (0,0) before the pointer coordinates are read.
    focusBtnRef.current?.focus({ preventScroll: true });
    if (isRoadTool && e.button === 0) {
      setIsRoadDragging(true);
      roadDragStartRef.current = coords;
      roadDragLastRef.current = coords;
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    if (isRoadDragging && isRoadTool) {
      ...
      const end = roadDragLastRef.current ?? getTileFromEvent(e);
      if (start && end && (start[0] !== end[0] || start[1] !== end[1])) {
        (onRoadDragEnd ?? onTileClick)(end[0], end[1]);
      }
    }
    roadDragStartRef.current = null;
    roadDragLastRef.current = null;
  ...
  return (
    <section className="city-2d-canvas relative flex flex-col h-full w-full overflow-hidden bg-slate-950 select-none" aria-label="Peta kota mode 2D" onPointerLeave={onTilePointerLeave}>
      ...
      <div className="flex-1 overflow-auto p-4 flex items-start justify-start">
        <div
          className="city-2d-grid relative flex-none border border-white/10 rounded-lg bg-slate-900/50 shadow-inner"
        ...
        >
          <canvas
            ...
            className="block max-w-none flex-none cursor-pointer"
            style={{ width: width * tileSize, height: height * tileSize }}
